...
begin
  ...
  if v_stock <> 7 then
    raise exception 'Smoke test falhou: entrada esperava estoque 7, recebeu %', v_stock;
  end if;

  select public.consume_part_for_service(
    v_order_id, v_part_id, 2, 11, 25, 'Smoke test: primeiro consumo'
  ) into v_item_one;
  ...
  if v_stock <> 5 or v_parts_cost <> 22 or v_parts_amount <> 50 or v_total_amount <> 100 then
    raise exception 'Smoke test falhou após primeiro consumo: estoque %, custo %, venda %, total %', v_stock, v_parts_cost, v_parts_amount, v_total_amount;
  end if;

  select public.consume_part_for_service(
    v_order_id, v_part_id, 1, 12, 30, 'Smoke test: segundo consumo'
  ) into v_item_two;
  ...
  if v_stock <> 4 or v_parts_cost <> 34 or v_parts_amount <> 80 or v_total_amount <> 130 then
    raise exception 'Smoke test falhou após segundo consumo: estoque %, custo %, venda %, total %', v_stock, v_parts_cost, v_parts_amount, v_total_amount;
  end if;

  begin
    perform public.record_inventory_movement(
      v_part_id, 'saida', 999, 0, null, null, 'Smoke test: saída inválida'
    );
  exception when others then
    if position('Estoque insuficiente' in sqlerrm) > 0 then
      v_failed_negative := true;
    else
      raise;
    end if;
  end;

  if not v_failed_negative then
    raise exception 'Smoke test falhou: estoque negativo não foi bloqueado';
  end if;

  if not public.remove_service_order_part(v_item_one) then
    raise exception 'Smoke test falhou: estorno retornou false';
  end if;
  ...
  if v_stock <> 6 or v_parts_cost <> 12 or v_parts_amount <> 30 or v_total_amount <> 80 then
    raise exception 'Smoke test falhou após estorno: estoque %, custo %, venda %, total %', v_stock, v_parts_cost, v_parts_amount, v_total_amount;
  end if;
  ...
  if exists (select 1 from public.parts_catalog where id = v_part_id)
    ...
    raise exception 'Smoke test falhou: limpeza dos dados temporários incompleta';
  end if;
end;
$$;